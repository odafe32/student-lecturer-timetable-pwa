            ...
            <div class="card user-info-card mb-3">
                <div class="card-body d-flex align-items-center">
                    <div class="user-profile me-3">
                        ...
                        <form action="{{ route('student.profile.image') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="user-info">
                        ...
                        @if ($student && $student->matric_number)
                            <p class="mb-0 text-muted">Matric Number: {{ $student->matric_number }}</p>
                        @endif
                        @if ($student && $student->level)
                            <p class="mb-0 text-muted">Level: {{ $student->level }}</p>
                        @endif
                    </div>
                </div>
            </div>
                    ...
                    <form action="{{ route('student.profile.update') }}" method="POST">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input class="form-control" id="phone" name="phone" type="text"
                                value="{{ old('phone', $student->phone_number ?? '') }}" placeholder="Phone Number">
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input class="form-control" id="department" type="text"
                                value="{{ $student->department->name ?? 'Not assigned' }}" readonly>
                        </div>

                        <div class="form-group mb-3">
                            <label class="form-label" for="level">Level</label>
                            <input class="form-control" id="level" type="text"
                                value="{{ $student->level ?? 'Not assigned' }}" readonly>
                        </div>
